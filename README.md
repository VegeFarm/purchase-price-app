...
마켓봄 거래명세서 링크를 입력하면 해당 HTML 안의 `hidden input name="statement"` 값의 JSON을 파싱하고, Google 스프레드시트 템플릿의 정해진 시트/컬럼에 **값만** 입력합니다.
...
전표번호는 마켓봄 statement JSON의 `slip.trade_slip_de_no`를 가장 먼저 사용합니다. 예: `S202607020007`. 없으면 `slip.trade_slip_no`를 사용하고, JSON에서 전표번호를 찾지 못할 때만 URL 마지막 값을 보조값으로 사용합니다. 총금액은 `amount.total`을 우선으로 읽습니다.
...
- `사용여부`가 `미사용`이면 매입기록과 미등록상품에는 넣지 않고 건너뜁니다.
...
- HTML 파일 업로드 기능은 제거되어 있습니다.
- 공개 링크에서 HTML 안에 `statement` 값이 내려오지 않는 경우에는 마켓봄 로그인/권한 또는 링크 접근 가능 여부를 먼저 확인하세요.
